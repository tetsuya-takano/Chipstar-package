...
namespace Chipstar.Downloads.CriWare
{
	...
	/// </summary>
	[Serializable]
	public sealed class CriVersionTable : IDisposable
	{
		//==================================
		//	class
		//==================================
		...
		/// ローカルのファイルを取得
		/// </summary>
		public static CriVersionTable ReadLocal( string path, Encoding encoding )
		{
			var isExist     = File.Exists( path );
			if( !isExist )
			{
				//なければ空データ
				return new CriVersionTable();
			}
			try
			{
				var contents = File.ReadAllText(path, encoding);
				return JsonUtility.FromJson<CriVersionTable>(contents);
			}
			catch (Exception e)
			{
				// 失敗したらとりあえず空でやり直し
